...
const { babel: BABEL_OPTS, dependencies } = require('./package.json')

// Treat all dependencies as ES2015+ (e.g., p-defer and event-target-shim)
const ES_PACKAGES = Object.keys(dependencies).filter(
  name => !name.startsWith('@babel/')
)

const NODE_MODULES = path.join(__dirname, 'node_modules')

const PACKAGE_ALIASES = {
  // Main is .mjs but that breaks the build
  'event-target-shim': path.join(
    NODE_MODULES,
